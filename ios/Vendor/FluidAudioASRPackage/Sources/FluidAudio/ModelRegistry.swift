...
public enum ModelRegistry {
    ...
    public static func resolveModel(_ repoPath: String, _ filePath: String) throws -> URL {
        let urlString = "\(baseURL)/\(repoPath)/resolve/\(modelRevision(for: repoPath))/\(filePath)"
        guard let url = URL(string: urlString) else {
            throw Error.invalidURL(urlString)
        }
        return url
    ...
    /// SpeakFree's two shipped recognizers are pinned to immutable Hugging Face commits. A model
    /// owner changing `main` must never change executable model bytes without an app update and a
    /// new accuracy/supply-chain review.
    public static func modelRevision(for repoPath: String) -> String {
        switch repoPath {
        case "FluidInference/parakeet-realtime-eou-120m-coreml":
