...
echo "speakfree transcription integration tests"
echo "--------------------------------------------"
...
if [ -z "$MODEL_PATH" ]; then
    ...
    MODEL_DIR="$HOME/.config/speakfree/models"
    mkdir -p "$MODEL_DIR"
...
TMPDIR_TEST=$(mktemp -d /tmp/speakfree-test.XXXXXX)
