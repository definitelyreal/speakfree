...
""")
    ...
    def test_default_fleet_stages_all_hosts_before_any_install(self):
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(events, ["initialize", "build", "stage:movie@100.82.136.101", "stage:ark",
                                  "install:local", "install:movie@100.82.136.101", "install:ark", "cleanup"])

    def test_build_or_remote_staging_failure_never_interrupts_any_host(self):
        for options in ({"BUILD_STATUS": 1}, {"FAIL_STAGE": "movie@100.82.136.101"},
                        {"FAIL_STAGE": "ark"}):
            with self.subTest(options=options):
    ...
    def test_remote_abort_stops_fleet_sequence_and_does_not_claim_cleanup(self):
        result, events, _ = self.run_flow(FLEET_HARNESS, FAIL_INSTALL="movie@100.82.136.101")
        self.assertNotEqual(result.returncode, 0)
